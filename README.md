...
1. 到 [**Releases**](../../releases) 页面下载最新版压缩包（内含 `B站课程进度追踪.exe`），解压到任意目录

2. 双击 `B站课程进度追踪.exe` 运行（无需安装 Python）
...
- **凭据加密存储**：SESSDATA 通过 Windows DPAPI 加密保存于 `.sessdata.bin`，只有当前 Windows 账户能解出；拷走文件或换账户均无法读取，磁盘上不存在明文（旧版明文 `.sessdata.txt` 首次启动自动迁移并删除）
- **本地数据最小化**：`tracked_videos.json` 只存"数字"（bvid、集数、秒数、时间戳）——课程标题、UP主、封面、分P列表不落盘，每次启动实时从B站拉取；即使文件泄露也看不出你在学什么
- **本地接口防护**：服务校验 `Host`（防 DNS rebinding）与 `Origin`/`Sec-Fetch-Site`（防其它网页 CSRF 调用本地端口）
- **凭据文件不可通过 HTTP 读取**：`.sessdata*` 与所有隐藏文件被静态服务直接拒绝
- **提交防护**：仓库内置 pre-commit 钩子（`hooks/`），凭据/数据/打包产物一旦进入暂存区立即拦截
...
| 问题 | 说明 |
|---|---|
| Tab 显示 "BV…XXXX" 占位 | 隐私设计：标题不落盘、实时从B站获取；断网或 SESSDATA 失效时暂缺，恢复后自动补齐 |
